...
def create_rule(rule_name, list_ids):
    ...
    data = {
        ...
        "action": "block",
        "traffic": " dns.location in {\"2a9940731f1b416db35f37c480e769db\"} "+" or dns.location in {\"2a9940731f1b416db35f37c480e769db\"} and ".join(f'any(dns.domains[*] in ${lst})' for lst in list_ids) + " ",
        "enabled": True,
    }
    status, response = cloudflare_gateway_request("POST", endpoint, body=json.dumps(data))
    return response["result"]
